...
export class CategoriaController {
  ...
  selecionarTodos = async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { id_perfil, padrao } = req.query;
      ...
      if (padrao === 'true') {
        const categorias = await this.service.selecionarPadrao();
        return res.status(200).json(categorias);
      }

      if (id_perfil) {
        const idPerfilConvertido = Number(id_perfil);

        if (isNaN(idPerfilConvertido)) {
          return res.status(400).json({ message: "ID do perfil inválido" });
        }

        const categorias = await this.service.selecionarPorPerfil(idPerfilConvertido);
        return res.status(200).json(categorias);
      }
  ...
  inserir = async (req: Request, res: Response) => {
    try {
      const { nome, id_perfil, padrao } = req.body;
